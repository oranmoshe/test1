...
ROM BYTE customChar[2][5] =
{
	{ 0x36, 0x49, 0x49, 0x49, 0x7f }, // button_not_pressed
	{ 0x7f, 0x49, 0x49, 0x49, 0x36 }, // button_pressed
};

void oledWriteIconRaw(int icon)
{

	//letter -= ' ';					// Adjust character to table that starts at 0x20
	WriteData(customChar[icon][0]);	// Write first column
	WriteData(customChar[icon][1]);	// Write second column
	WriteData(customChar[icon][2]);	// Write third column
	WriteData(customChar[icon][3]);	// Write fourth column
	WriteData(customChar[icon][4]);	// Write fifth column
	WriteData(0x00);					// Write 1 column for buffer to next character
	return;
}

void oledWriteIcon(int icon, unsigned char page, unsigned char column) {
	BYTE i;
	//page = page + 0xB0;
	WriteCommand(page);
	column += OFFSET;
...
ROM BYTE g_pucFont[95][5] =
{
	...
	{ 0x08, 0x04, 0x72, 0x04, 0x08 }, // "   unpressed_button_up
	{ 0x08, 0x0c, 0x7e, 0x0c, 0x08 }, // #   pressed_button_up
	{ 0x10, 0x20, 0x4e, 0x20, 0x10 }, // $   unpressed_button_down
	{ 0x10, 0x30, 0x7e, 0x30, 0x10 }, // %   pressed_button_down
	{ 0x7f, 0x41, 0x22, 0x14, 0x08 }, // &   unpressed_button_right
	{ 0x7f, 0x7f, 0x3e, 0x1c, 0x08 }, // '   pressed_button_right
	{ 0x7f, 0x7f, 0x3e, 0x1c, 0x08 }, // (   pressed_button_right
	{ 0x08, 0x14, 0x22, 0x61, 0x7f }, // )   unpressed_button_left
	{ 0x08, 0x1c, 0x3e, 0x7f, 0x7f }, // *   pressed_button_left
	{ 0x06, 0x09, 0x09, 0x06, 0x00 }, // +   c
	{ 0x36, 0x49, 0x49, 0x49, 0x7f }, // ,   unpressed_button
	...
	{ 0x47, 0x49, 0x49, 0x49, 0x31 }, // S   0X53
